...
import { deleteDatabase, getAllConfigs, saveConfig, type RecoConfig } from "@/lib/db";
...
const toOptions = (list: string[]): Option[] => list.map((item) => ({ label: item, value: item }));
...
export default function ConfigPage() {
  const loadConfigs = async () => {
    try {
      ...
      setSavedConfigs(configs.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()));
    } catch (e) {
  ...
  // API Mutation
  const [getFeed, { data: feedData, isLoading: isFeedLoading, error: feedError }] =
    useGetFeedMutation();
  ...
  const handleSave = async () => {
    ...
    const config: Omit<RecoConfig, "id" | "createdAt"> = {
      name,
      location: {
        country,
        region:
          country.toLowerCase().includes("united states") || country === "US" || country === "USA"
            ? region
            : undefined,
      },
      domains,
          ...
          <div className="space-y-6 h-full overflow-y-auto lg:pr-2 pb-4 custom-scrollbar">
              ...
              <div className="flex gap-2 items-end">
                ...
                <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
                  <DialogTrigger asChild>
                    <Button variant="destructive" size="icon" title="Delete Database">
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        ...
                        configurations from your browser&apos;s database.
                      </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
            ...
            <Card className="border-border shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Configure the weights and filters for the recommendation engine.
                </CardDescription>
              </CardHeader>
        ...
        <div className="relative h-[calc(100vh-8rem)] sticky top-8">
          <div className="space-y-6 h-full overflow-y-auto lg:pr-2 pb-4 custom-scrollbar">
            <div className="flex items-center justify-between sticky top-0 bg-background z-10 py-2">
              <h2 className="text-2xl font-semibold tracking-tight">Recommendation Feed</h2>
              <Button onClick={handleGetFeed} disabled={isFeedLoading}>
                {isFeedLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Get Feed
              </Button>
            </div>
            ...
            <div className="space-y-4">
              ...
              {/* Empty State */}
              {!isFeedLoading && (!feedData || feedData.length === 0) && !feedError && (
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <CardTitle className="text-lg">{card.operational_name}</CardTitle>
                    ...
                    {card.target_markets && card.target_markets.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {card.target_markets.map((market) => (
                          <Badge key={market} variant="outline" className="text-xs font-normal">
                            {market}
                          </Badge>
                        ))}
                      </div>
