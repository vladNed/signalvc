...
- **Build for today, not a hypothetical future.** Solve the current requirement directly. When requirements change, refactor then — don't pre-engineer extension points, feature flags, or plugin systems before they are needed.
- **Keep indirection shallow.** A developer should be able to trace from a button click to the actual API call in two or three hops. Avoid layering abstractions (service → adapter → provider → client) when a single module would do.
