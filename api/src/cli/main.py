...
def parse_target_markets(val: str) -> str:
    ...
    return json.dumps([market.strip() for market in stripped_val.split("|") if market.strip()])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Append Veridion data to Supabase database.")
    parser.add_argument("table", type=str, help="The target table name to append data to.")
    args = parser.parse_args()

    root = pathlib.Path(".")

    with status.Status(f"Searching for [bold blue]{args.table}[/bold blue] data file...", console=log):
        data_file = root / "data" / f"{args.table}.parquet"
        if not data_file.exists():
            log.print(f"[bold red]Error:[/bold red] Data file [bold]{data_file}[/bold] does not exist.")
            return
    ...
    log.print(f"  - Memory usage: {df.memory_usage(deep=True).sum() / (1024 ** 2):.4f} MB\n")
    ...
    with status.Status("Adding data to Supabase...", console=log):
        with engine.begin() as conn:
            df.to_sql(name=args.table, con=conn, if_exists="append", index=False, method="multi", chunksize=10000)
