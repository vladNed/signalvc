from rich import console, status, progress
import pandas as pd
import psycopg2
import json
...
def main():
    ...
    with status.Status("Loading Veridion data...", console=log):
        df = pd.read_parquet("data/veridion-data-cleaned.snappy.parquet", engine="pyarrow")

    log.print("▶ Loaded data:")
    log.print(f"  - Rows: {len(df)}")
    log.print(f"  - Columns: {df.columns.tolist()}")
    log.print(f"  - Memory usage: {df.memory_usage(deep=True).sum() / (1024 ** 2):.2f} MB\n")
    ...
    with status.Status("Adding data to Supabase...", console=log):
        with engine.begin() as conn:
            df.to_sql(name="startup", con=conn, if_exists="append", index=False, method="multi", chunksize=10000)
